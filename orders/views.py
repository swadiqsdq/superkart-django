...
def place_order(request,total=0,quantity=0):
    ...
    # if cart count is less than 0 then redirect to store
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
             # // store all the billing information to the order table
            data =  Order()
            ...
            data.tax = tax
            data.ip = request.META.get('REMOTE_ADDR') #// get user ip address
            data.save()
            ...
            current_date = d.strftime('%Y%m%d') #//20251031
